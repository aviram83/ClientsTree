...
- **Node re-parenting (Move):** `NodeForm.tsx`'s edit mode shows a destructive "Move..." button (hidden for the root node, which has no valid target) that swaps the modal's content in place for `MoveNodePicker.tsx` rather than stacking a second modal. The picker walks the full tree, greying out the node itself and its descendants as invalid targets (`client/src/lib/nodeMove.ts`'s `getDescendantIds`/`isValidMoveTarget`, mirroring the server's `wouldCreateCycle`), shows how many descendants will move, and — via `computeHasSupervisorAncestorAtParent` and the house-reclassification-preview counter in `nodeMove.ts` — how many of them will switch between the Personal House and Supervisor House (a node only counts if it's actually visible in a house per `flattenVisibleHouseNodes`'s rules; SUPERVISOR nodes are excluded since their own membership doesn't depend on ancestry). Confirming calls `treeStore.moveNode(id, newParentId)`, which hits `PATCH /tree/:id/move` and refetches the tree.
