...
- **Routing:** React Router v6 in `client/src/Router.tsx`. `/dashboard`, `/houses/clients`, and `/houses/supervisors` are protected by `ProtectedRoute`.
- **Nav config:** `client/src/config/navConfig.ts` — `NAV_ITEMS` is a `NavEntry[]` union of flat `NavItem` (label/path/icon) and `NavGroup` (label/icon/children: `NavItem[]`), disambiguated by `isNavGroup()`. `UserSideMenu.tsx` renders groups as an expandable submenu. Labels are Hebrew ("עץ לקוחות", "בתים" grouping "ניקוד אישי" and "ניקוד מפקחים").
