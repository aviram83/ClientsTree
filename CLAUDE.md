...
- **API layer:** `client/src/api/api.ts` — axios instance with request interceptor (injects Bearer token) and response interceptor: a 401 with message `'Token is not valid'` triggers `logout()`; any other 4xx/5xx response shows the server's message via `showErrorModal`; a request that gets no response at all (network failure, or a Render cold-start timeout) shows `i18n.t('common.networkError')` instead of failing silently. Functions in `client/src/api/index.ts`, including `moveNode(id, newParentId)` → `PATCH /tree/:id/move`.
...
- **Node re-parenting (Move):** `NodeForm.tsx`'s edit mode shows a destructive "Move..." button (hidden for the root node, which has no valid target) that swaps the modal's content in place for `MoveNodePicker.tsx` rather than stacking a second modal. The picker walks the full tree, greying out the node itself and its descendants as invalid targets (`client/src/lib/nodeMove.ts`'s `getDescendantIds`/`isValidMoveTarget`, mirroring the server's `wouldCreateCycle`), shows how many descendants will move, and — via `computeParentMoveContext` (which returns both the `hasSupervisorAncestor` flag and depth for a given parent id) and the house-reclassification-preview counter in `nodeMove.ts` — how many of them will switch between the Personal House and Supervisor House. `computeHouseReclassificationPreview` returns separate `toSupervisorCount`/`toPersonalCount` (both can be non-zero for the same move) because a node's membership can change for two independent reasons: a non-supervisor's ancestor-flag crossing, or a SUPERVISOR node's own depth crossing the depth-1 boundary, which swaps it between the two houses (see House views below) — a node only counts if it's actually visible in a house per `flattenVisibleHouseNodes`'s rules. Confirming calls `treeStore.moveNode(id, newParentId)`, which hits `PATCH /tree/:id/move` and refetches the tree.
...
- **House views:** `ClientsHouse/HouseView.tsx` is the shared read-only visualization of tree data as a house — a roof (full-price clients) plus a 2x2 grid of rooms for each `PercentageLevel` discount tier, laid out by `client/src/lib/houseLayout.ts` and drawn by `HouseBackground.tsx`. Room/roof fill colors use an ascending-lightness scale keyed by `PercentageLevel` (roof darkest, higher discount levels progressively lighter), defined in `client/src/config/percentageConfig.ts`. Every client renders as an identical black-bordered square via `HouseNode.tsx` (unlike `CustomNode.tsx`'s per-status shapes) with its name split across two lines; nodes are non-interactive (no drag, no click, no React Flow Handles). `flattenVisibleHouseNodes` in `houseLayout.ts` filters out clients that are inactive or whose `percentageLevel` is unset/hidden (`LEVEL_6`); an optional `HouseMembershipFilter` further restricts which visible nodes are included, based on whether a SUPERVISOR ancestor sits above the node and the node's own depth (root-level nodes = depth 0, their direct children = depth 1, etc. — tracked alongside the ancestor flag in the same recursive pass). Two pages consume `HouseView`: `ClientsHouseView.tsx` (route `/houses/clients`, "ניקוד אישי" / Personal House) uses `isClientsHouseMember` — a SUPERVISOR only belongs here if it's a direct child of the tree's single root (depth === 1); a SUPERVISOR nested deeper (under another supervisor, or under a plain client) shows only in the Supervisor House. Every non-supervisor client with no SUPERVISOR ancestor still qualifies regardless of depth. `SupervisorHouseView.tsx` (route `/houses/supervisors`, "ניקוד מפקחים" / Supervisor House) uses `isSupervisorHouseMember` — every SUPERVISOR *except* the depth-1 ones (`depth !== 1`), plus every non-supervisor client that has a SUPERVISOR ancestor. The two filters partition supervisors cleanly: a depth-1 supervisor appears only in the Personal House, every deeper supervisor only in the Supervisor House, and none appears in both. `NodeForm.tsx` defaults the discount `<select>` to `LEVEL_4` the moment `SUPERVISOR` status is selected (a one-time default, not a lock), but the field stays freely editable to any level — the server no longer enforces a SUPERVISOR/LEVEL_4 pairing, so a SUPERVISOR is not guaranteed to land in the 50% room; `groupNodesByPercentageLevel` keys room placement purely off each node's actual `percentageLevel`. An inactive supervisor doesn't count as an ancestor, so its active clients fall back to the Personal House rather than disappearing.
...
- **Routes:** `server/src/routes/` → `auth.routes.ts` (`/api/auth`, includes public `POST /forgot-password` and `POST /reset-password`, plus JWT-protected `GET /me` and `PATCH /me` for reading/updating the profile's language preference) and `tree.routes.ts` (`/api/tree`, JWT-protected, including `PATCH /:id/move` for re-parenting a node — see Node re-parenting below).
...
- **Database:** `server/prisma/schema.prisma` — `User` and `TreeNode` models. `TreeNode` is self-referential (parentId) for hierarchy, with an `@@index([userId])` (added for the move endpoint's cycle check, which scans all of a user's nodes). Cascading deletes are configured. `User` also carries `resetTokenHash`/`resetTokenExpiresAt` for the password-reset flow (the raw token is never stored, only its hash).
- **Node re-parenting:** `moveNode` in `tree.controller.ts` handles `PATCH /tree/:id/move` (`{ newParentId }` body) — loads both nodes via `findOwnedNode` (404 if either is missing/not owned), rejects a self-move, then loads the caller's full node set (`{id, parentId}` only) and runs `wouldCreateCycle` (`server/src/utils/tree.ts`) before updating `parentId`; a concurrent delete of either node mid-flight (Prisma `P2025`/`P2003`) is caught and surfaced as 404, not 500. `addNode` also gained a single-root guard: a `parentId`-less create is rejected with 400 if the user already has a root node, since `getTree`/`TreeVisualizer`/`HouseView` all assume exactly one root per user.
- **XSS:** Server uses the `xss` library for sanitization.
- **Validation rules:** `server/src/utils/validation.ts` also exports `isValidLanguage()`, checking against `SUPPORTED_LANGUAGES` (`'he' | 'en'`), enforced by `updateProfile` in `auth.controller.ts` on `PATCH /api/auth/me`. A SUPERVISOR node's discount level is no longer coupled to `PercentageLevel.LEVEL_4` — it defaults there client-side on selection but is independently editable to any level, and the server does not enforce a SUPERVISOR/LEVEL_4 pairing (the historical `server/scripts/backfill-supervisor-level.ts` script, which backfilled rows to LEVEL_4 for the now-removed rule, is no longer needed but is left in place).
...
- **Tree mutations refetch the full tree** — `treeStore` calls `fetchTree()` after every add/update/delete/move (no optimistic updates).
