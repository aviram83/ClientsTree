...
// Shared ReactFlow/layout plumbing for every house visualization. Each house
// (ClientsHouseView, SupervisorHouseView) is a thin wrapper around this with
// its own membership filter — the roof+room grid math and rendering are
// identical, only which nodes qualify differs.
const HouseView = ({ filter }: HouseViewProps) => {
  ...
  useEffect(() => {
    const visibleNodes = flattenVisibleHouseNodes(tree, filter);
